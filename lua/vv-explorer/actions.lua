...
function M.node_under_cursor(state)
  ...
  -- filter 模式：line 1 就是首个匹配行（没有 root header）
...
function M.open(state)
  ...
  -- 过滤模式下 dir 节点的语义：退出过滤并展开到该目录（子节点未装载）
  if state.filter and state.filter.active and node.is_dir then
    ...
    Render.render(state)
    local lnum = state.path_to_row[node.path]
    if lnum then vim.api.nvim_win_set_cursor(state.win, { lnum, 0 }) end
    return
  end
  ...
  -- 文件：升级预览（不再删），切焦点到主窗口打开
  Preview.promote(state)
  local main = Preview.find_main_win(state.win)
  if not main then
    -- 没主窗口：只有 explorer 一个窗口（用户先 :only 或 <leader>ba 后只剩树）。
    -- 从 explorer 窗口 :vsplit 出的新窗口会继承 explorer 的隐藏 winopts
    -- (number=false 等)。手动 show_chrome 把新窗口的装饰拉回全局默认。
...
function M.close_node(state)
  ...
  if node.parent and node.parent ~= state.root then
    ...
    Render.render(state)
    local lnum = state.path_to_row[node.parent.path]
    if lnum then vim.api.nvim_win_set_cursor(state.win, { lnum, 0 }) end
  end
end
...
local function open_in(state, cmd)
  ...
  -- 同 M.open：从 explorer split 出来的新窗口要把装饰拉回全局默认，
  -- 否则 number=false 等会被继承
...
local function refilter(state)
  ...
  if not f.index then
    f.matched = { abs = {}, rels = {}, positions = {} }
  else
    f.matched = Filter.match(f.index, state.root.path, f.query)
  end
  Render.render(state) -- 更新 state.filter.match_count
  if f.on_match_count_update then pcall(f.on_match_count_update) end
end
...
function M.start_filter(state)
  ...
  -- 第一次触发：异步建全树索引（fd 不存在 → 提示 + 直接退出过滤态）
  if not f.index and not f.index_building then
  ...
  Prompt.open(state, f.query,
    function(q) -- on_change
      f.query = q
      refilter(state)
    end,
    function(q) -- on_submit：保持 filter，把光标落到首个 match
      f.query = q
      refilter(state)
      if vim.api.nvim_win_is_valid(state.win) then
        vim.api.nvim_set_current_win(state.win)
        local first = f.matched and f.matched.abs[1]
        if first then
          local lnum = state.path_to_row[first]
          if lnum then vim.api.nvim_win_set_cursor(state.win, { lnum, 0 }) end
        end
      end
    end,
    function() -- on_cancel：清除 filter
      M.clear_filter(state)
      if vim.api.nvim_win_is_valid(state.win) then
        vim.api.nvim_set_current_win(state.win)
      end
