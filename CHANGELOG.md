...
- **主窗恢复策略**：新增 `restore_main_win(win)` 配置；属性页关闭时若预览前的 buffer 已失效，可由调用方接管目标窗口，否则依次回退到 vv-dashboard 与空白 buffer

### Fixed

- **关闭面板遗留属性页**：关闭 explorer 时同步撤下目录与二进制属性页、取消在途目录统计，并恢复预览链开始前的 buffer；属性页已被外部 `:edit` 提前回收时也会清理扫描与预览追踪，不再遗留后台任务和失效引用

## 0.3.4 - 2026-08-10

### Changed

- **确认交互**：删除、永久删除、清空回收站与 `X` 执行文件改用分层确认浮窗，展示目标、工作目录与命令，并统一键位和语义高亮

### Fixed

- **确认安全**：确认等待期间校验 explorer 窗口、buffer、root 代际及目标快照；切根、关闭面板或发起新操作会取消旧确认，避免过期确认误删或误执行
- **执行稳定性**：执行前校验目标路径、工作目录和解析命令；运行器或终端启动失败时改为通知，不再抛错或创建无效终端
- **回收站操作安全**：永久删除与清空前复核条目身份及文件元数据，条目被外部修改或新增时取消操作，避免误删
